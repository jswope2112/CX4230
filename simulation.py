...
class Simulation():
    ...
    def __init__(self, time_factor, smart_lights, smart_cars, iterations):
    ...
    def run(self):
        ...
        for intersection in self.intersections:
            self.s.enter(t * self.time_factor, 1, intersection.cycle, [self.time_factor])
            t += 5
        self.s.run()
    ...
    def auto_arrivals(self):
        ...
        for intersection in self.intersections:
            for side in intersection.sides:
                if side.on_boundary:
                    self.s.enter(t * self.time_factor, 1, side.arrival, [int(np.random.normal(side.arrival_distribution[0], side.arrival_distribution[1]))])
                    t += 1
...
if __name__ == '__main__':
    ...
    sim = Simulation(.001, False, False, 1)
    sim.run()
